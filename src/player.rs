...
impl Player {
    ...
    pub fn start_playing(&mut self, loops: Loops, shuffle: bool, _file_paths: &Vec<String>) {
        match loops {
    ...
    fn play_audio_clips(&mut self, shuffle: bool, file_paths: &Vec<String>) {
        let mut indices: Vec<usize> = (0..file_paths.len()).into_iter().collect();
        if shuffle {
            let mut rng = rng();
    ...
    fn play_audio_clips_in_loop(&mut self, loops: Loops, shuffle: bool, _file_paths: &Vec<String>) {
        match loops {
            Loops::Infinite => loop {
                self.play_audio_clips(shuffle, _file_paths)
            },
            Loops::Count(count) => {
                (0..count as usize).for_each(|_| self.play_audio_clips(shuffle, _file_paths))
            }
